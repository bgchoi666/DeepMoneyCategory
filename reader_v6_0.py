...
def get_dataset(raw_df, config):
  ...
  config.train_start2 = raw_df['date'].values[config.train_start2_index]
  ...
  train_target_raw = raw_df.values[max(0, config.train_start1_index - config.step_interval*(config.num_steps - 1) - config.predict_term): config.train_end1_index + 1, config.input_size+2]
  ...
  #train target data created
  train_target_raw = raw_df.values[max(0, config.train_start2_index - config.step_interval*(config.num_steps - 1) - config.predict_term): config.train_end2_index + 1, config.input_size+2]
  ...
  if config.test_start_index > 0 and config.test_start_index < config.test_end_index:
    test_target_raw = raw_df.values[config.test_start_index - config.step_interval * (config.num_steps-1): config.test_end_index + 1 + config.predict_term, config.input_size+2]
    test_target = []
...
def producer(raw_data, num_steps, step_interval, input_size, output_size):
  ...
  if path.Config().input_size == 809: x[:, num_steps - 1, 786:input_size] = 0
...
def make_norm(raw_data, conversion, predict_term, input_size, days):
  ...
  for i in range(days-1, raw_data.shape[0]):
    ...
    if i + predict_term < raw_data.shape[0]:
      ...
      if conversion == 'sign' : r.append(np.sign(raw_data[i + predict_term, input_size] - raw_data[i, input_size]))
    else: r.append(0)
    norm_data.append(r)
