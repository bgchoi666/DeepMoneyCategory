...
file_name = 'C:/Users/user/Desktop/DeepMoney_Category/DeepMoneyData/new/kospi200f_200302.csv'
        ...
        for future_day in future_days:
            ...
            for n_timestep in n_timesteps:
                for time_interval in time_intervals:
                    ...
                    early_stopping = learn.EarlyStopping(patience=1, verbose=1)

                    train_data, test_data = prepro.get_train_test_data(df, target_column,
                    ...
                    autoencoder = models.AutoEncoderLSTM(n_timestep,input_size,regularizers_alpha=0.01,drop_rate=0.3)
                    ...
                    for iteration in range(500):
                        ...
                        gradients = models.gradient(autoencoder, 'mean_square', None, batch_input, batch_output)
                        optimizer.apply_gradients(zip(gradients, autoencoder.trainable_variables))

                        if iteration % 1 == 0:
                            loss = models.loss_mean_square(autoencoder, None, batch_input, batch_output)
                    ...
                    encoding = keras.Model(inputs=autoencoder.input,outputs=autoencoder.get_layer('HiddenLayer2').output)
                    n_input = 256



                    model = models.ResLSTM2noise(n_timestep,n_input,n_unit,regularizers_alpha=0.01,drop_rate=0.5, gpu=1)
                    ...
                    for iteration in range(300):
                        ...
                        batch_input = encoding(batch_input, training = False)
                        ...
                        if iteration %10 == 0:
                            loss = models.loss_mean_square(model, None, batch_input, batch_output)
                            train_MSE = models.evaluate(model, batch_input, batch_output)
                            #test_MSE =  models.evaluate(model, test_input[:100], test_output[:100])
                            test_MSE =  models.evaluate(model, encoding(batch_test_input,training = False), batch_test_output)
                    ...
                    train_encoder = learn.predict_batch_test(encoding, train_x,100)
                    test_encoder = learn.predict_batch_test(encoding, test_x,100)
