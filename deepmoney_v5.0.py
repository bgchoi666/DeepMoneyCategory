...
def no_gradual_train(k, M2M, config):
    ...
    # first, set the kelly loss to 0 after saving
    kelly = config.beta
    config.beta = 0
    ...
    #if config.kelly == true, reinforce the training to increase kelly index
    if config.kelly:
        config.iter_steps = config.gradual_steps
        # second, restore the saved kelly loss rate and set the RRL loss rate to 0
        config.beta = kelly
        config.alpha = 0
        if config.train_start1 < config.train_end1: model.train(M2M, train_data1, config)
        if config.train_start2 < config.train_end2: model.train(M2M, train_data2, config)
...
def gradual_train(k, M2M, config):
    ...
    first_gradual = True
    rf = []
    RRL_rate = config.alpha
    kelly_rate = config.beta
    while config.train_end1_index >= config.test_end_index:
        ...
        if config.kelly:
            # create and train model without kelly
            # set the weighted RRL loss rate with saved one
            config.alpha = RRL_rate
            config.beta = 0
            model.train(M2M, train_data2, config)

            # train the already trained model with kelly
            # set the weighted kelly loss rate with saved one
            config.alpha = 0
            config.beta = kelly_rate
            model.train(M2M, train_data2, config)

        #if config.kelly:
        #    # create and train model without kelly
        #    config.shufflen = True
        #    config.iter_steps = config.iter_steps_max
        ...
        #    config.iter_steps = config.gradual_steps
        #    M2M = model.create_estimator_kelly(config, M2M.model_dir)
        #    model.train(M2M, train_data2, config)
        else:
            model.train(M2M, train_data2, config)
...
def main(_):
    ...
    if ensemble:
       ...
       r.write("\n ensemble, gradual, RRL, kelly, layers, hidden_size, batch_size, iteration\n" +
               ...
               str(config[0].RRL) + "-" + str(config.alpha) + "," + str(config.kelly) + "-" + str(config.beta) + "," +
       ...
       r.write("\n ensemble, gradual, RRL, kelly, time steps, interval, layers, hidden_size, batch_size, iteration\n" +
               ...
               str(config.RRL) + "-" + str(config.alpha) + "," + str(config.kelly) + "-" + str(config.beta) + "," +
