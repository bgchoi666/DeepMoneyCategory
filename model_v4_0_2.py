...
import reader_v4_0_2 as reader
    ...
    if config.kelly:
        ...
        for i in range(batch_size):
            r = tf.cond((predict_prices[i, num_steps - 1, 0] - base_prices[i, num_steps - 1, 0]) *
                        ...
                        lambda: return_zero())
            if r == 1:
                profits.append(tf.abs(target_prices[i, num_steps - 1, 0] + predict_prices[i, num_steps - 1, 0] - predict_prices[i, num_steps - 1, 0] - base_prices[i, num_steps - 1, 0])/base_prices[i, num_steps - 1, 0])
            else:
                losses.append(tf.abs(target_prices[i, num_steps - 1, 0] + predict_prices[i, num_steps - 1, 0] - predict_prices[i, num_steps - 1, 0] - base_prices[i, num_steps - 1, 0])/base_prices[i, num_steps - 1, 0])
            #profit_loss.append(tf.sign((predict_prices[i, num_steps - 1, 0] - base_prices[i, num_steps - 1, 0]) *
            #                           (target_prices[i, num_steps - 1, 0] - base_prices[i, num_steps - 1, 0])) *
            #                   tf.abs(target_prices[i, num_steps - 1, 0] - base_prices[i, num_steps - 1, 0]))
            #avg_profit = (tf.reduce_sum(profit_loss) + tf.reduce_sum(losses)) / tf.size(profits, out_type=tf.dtypes.float32)
            #avg_loss = (tf.reduce_sum(profits) - tf.reduce_sum(profit_loss)) / tf.size(losses, out_type=tf.dtypes.float32)

        avg_profit = tf.reduce_mean(profits)
        avg_loss = tf.reduce_mean(losses)

        loss3 =  avg_profit - avg_loss
    else:
        loss3 = 0

    # minimize the loss of prediction network and maximize downside sharp ratio of RRL
    # and maximize kelly's invest rate
...
def model_fn_kelly(features, labels, mode, params):
    #customized many to many LSTM model function
    ...
    for i in range(batch_size):
        r = tf.cond((predict_prices[i, num_steps-1, 0] - base_prices[i, num_steps-1, 0]) *
                   ...
                   lambda: return_one(),
                   lambda: return_zero())
        if r == 1: profits.append(tf.abs(target_prices[i, num_steps-1, 0] + predict_prices[i, num_steps-1, 0] - predict_prices[i, num_steps-1, 0] - base_prices[i, num_steps-1, 0]))
        else:      losses.append(tf.abs(target_prices[i, num_steps-1, 0] + predict_prices[i, num_steps-1, 0] - predict_prices[i, num_steps-1, 0] - base_prices[i, num_steps-1, 0] ))
        #profit_loss.append(tf.sign((predict_prices[i, num_steps-1, 0] - base_prices[i, num_steps-1, 0]) *
        #                           (target_prices[i, num_steps-1, 0] - base_prices[i, num_steps-1, 0])) *
        #                   tf.abs(target_prices[i, num_steps-1, 0] - base_prices[i, num_steps-1, 0]))
    #avg_profit = (tf.reduce_sum(profit_loss) + tf.reduce_sum(losses)) / num_of_profits
    #avg_loss = (tf.reduce_sum(profits) - tf.reduce_sum(profit_loss)) / num_of_losses
    avg_profit = tf.reduce_mean(profits)
    avg_loss = tf.reduce_mean(losses)
    ...
    # loss is the difference between average profit and loss
    loss =  loss - config.gamma * (avg_profit - avg_loss)
...
def create_estimator(config, dir):
    ...
    model_dir_cont = path.model_dir + "/" + path.market + "_" + str(config.predict_term) + "_" + str(config.step_interval) + "_" + str(config.num_steps) + "_" + \
                     str(config.hidden_size1) + "_" + str(config.batch_size) + "_" + path.model_num
    if dir == "" or config.model_reset == True: # if True, new model creation mode
        try:
...
def create_estimator_kelly(config, dir):
    #configuration parameter --> model parameter
    model_params = {"config": config}
    ...
    # already trained model is reinforced to increase kelly rate
    if os.path.isdir(dir) : # reuse existing model
        return tf.estimator.Estimator(model_fn=model_fn_kelly, model_dir=dir, params=model_params, config=my_checkpointing_config)
    else :
