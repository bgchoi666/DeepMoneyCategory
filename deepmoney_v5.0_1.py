...
import reader_v5_0_1 as reader
import path_v5_0_1 as path
import model_v5_0_1 as model
...
def gradual_train(k, M2M, config):
    ...
    while config.train_end1_index >= config.test_end_index:
        ...
        #if config.train_end2_index - config.train_start2_index > 500:
        ...
        if config.train_end2_index - config.train_start2_index > 500:
            config.train_start2_index += config.gradual_term
        config.train_end2_index += config.gradual_term
        config.test_start_index += config.gradual_term
        config.test_end_index += config.gradual_term
        if config.train_end2_index > config.train_end1_index:
            config.train_end2_index = config.train_end1_index
        if config.test_end_index > config.train_end1_index:
            config.test_end_index = config.train_end1_index
        if config.test_start_index > config.train_end1_index or config.train_start2_index > config.train_end1_index:
            break
...
def main(_):
    ...
    log.info("ensemble : " + str(config.ensemble))
    log.info("gradual : " + str(config.gradual))
    log.info("rmode : " + str(config.rmode))
    log.info("base : " + config.base)
    log.info("init_scale : " + str(config.init_scale))
    log.info("learning_rate : " + str(config.learning_rate))
    log.info("predict_term : " + str(config.predict_term))
    log.info("setup_interval : " + str(config.step_interval))
    log.info("num_steps : " + str(config.num_steps))
    log.info("num_layers : " + str(config.num_layers))
    log.info("hidden_size1 : " + str(config.hidden_size1))
    log.info("hidden_size2 : " + str(config.hidden_size2))
    log.info("batch_size : " + str(config.batch_size))
    log.info("input_size : " + str(config.input_size))
    log.info("output_size : " + str(config.output_size))
    log.info("rnn_mode : " + config.rnn_mode)
    log.info("norm_days : " + str(config.norm_days))
    log.info("conversion : " + config.conversion)
    log.info("iter_steps_max : " + str(config.iter_steps_max))
    log.info("iter_steps : " + str(config.iter_steps))
    log.info("gradual_steps : " + str(config.gradual_steps))
    log.info("gradual_term : " + str(config.gradual_term))
    log.info("alpha : " + str(config.alpha))
    log.info("beta : " + str(config.beta))
    log.info("model_reset : " + str(config.model_reset))
    log.info("shuffle : " + str(config.shuffle))
    log.info("total term :" + config.train_start1 + "~" + config.train_end1)
    log.info("train term :" + config.train_start2 + "~" + config.train_end2)
    log.info("test term :" + config.test_start + "~" + config.test_end)
    ...
    if ensemble:
       ...
       result_file = path.result_dir + "/" + path.market + "_" + "alpha" + str(config[0].alpha) + "beta" + str(config[0].beta) + \
                     "_ensemble_" + str(config[0].predict_term) + "day_" + \
                     "reinf_" + str(config[0].rmode) + "_" + datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d%H-%M-%S') + ".csv"
       ...
       list_index = list(res[0]["iiindex_today"].values)
       ...
       for i in range(tot_num_thread):
           ...
           list_index = list(res[i]["iiindex_today"].values)
           average_index = sum(list_index) / len(list_index)
           kelly_rate = average_index * (accuracy / -average_loss - (1 - accuracy) / average_profit)
       ...
       avg_index = sum(list(res["iiindex_today"].values)) / len(list(res["iiindex_today"].values))
