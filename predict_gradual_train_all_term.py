...
import reader_gradual_train_all_term as reader
   ...
   """
    ...
    for i in range(num_steps*batch_size):
        if not shuffle :
            ...
            if i > num_steps and i % num_steps == num_steps - 1 : logit_pre.append(logit[i-num_steps])
            else: logit_pre.append(logit[i])
...
def create_estimator(config):
    ...
    model_dir = path.model_dir +  "/" + path.market + "_" + "alpha" + str(config.alpha) + "beta" + str(config.beta) + "_" + str(config.predict_term) + "_" + \
...
def main(_):
    ...
    accuracy, _, _, _ = calculate_recall_precision(index_real_tot, index_pred_tot, index_today_tot)
    piacc = cal_piacc(std_tot, index_pred_tot, index_real_tot, profits_tot)
