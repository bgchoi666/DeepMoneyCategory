...
def no_gradual_train(k, M2M, config, log):
    ...
    # train and evaluate the model every 100 steps
    i = 0
...
def gradual_train(k, M2M, config, log):
    ...
    for i in range(len(config.grad_train_terms) - 2):
        if i > 0:  config.iter_steps_max = 300
    ...
    if FLAGS.ensemble:
       ...
       for j in range(tot_num_thread):
           ...
           if FLAGS.gradual:
               ...
               #t[j] = Process(target=gradual_train, args=(j, M2M[j], config[j], log))
           else: 
               t[j] = threading.Thread(target=no_gradual_train, args=(j, M2M[j], config[j], log))
               #t[j] = Process(target=no_gradual_train, args=(j, M2M[j], config[j], log))
           t[j].start()
       ...
       index_pred_avg = [0 for i in range(len(today_tot))]
       profits_avg = [0 for i in range(len(today_tot))]
       pred_values_avg = [0 for i in range(len(today_tot))]
       for i in range(0, 4):
       ...
       if FLAGS.gradual:
           gradual_train(0, M2M, config, log)
       else:
           no_gradual_train(0, M2M, config, log)

       log.info(datetime.now().strftime("%Y %m %d %H %M %S"))
       log.info("======================no-ensemble train-predict end, saving the results ===========================")
    
       # write the total results to a file
       comp_results = {"date_base": today_tot, "real": target_values_tot, "prediction": pred_values_tot[0], "index_today": index_today_tot,
                       "index_real": index_real_tot, "index_pred": index_pred_tot[0], "date_pred": date_tot, "loss_profits": profits_tot[0],
                       "std": std_tot}
       ...
       accuracy, _, _, _ = calculate_recall_precision(index_real_tot, index_pred_tot[0], index_today_tot, profits_tot[0])
       piacc = cal_piacc(std_tot, index_pred_tot[0], index_real_tot, profits_tot[0])
