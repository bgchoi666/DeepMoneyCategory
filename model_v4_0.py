...
def model_fn(features, labels, mode, params):
    ...
    for i in range(batch_size):
        r = tf.cond((predict_prices[i, num_steps - 1, 0] - base_prices[i, num_steps - 1, 0]) *
                    ...
                    lambda: return_zero())
        if r == 1:
            profits.append(tf.abs(target_prices[i, num_steps - 1, 0] - base_prices[i, num_steps - 1, 0]))
        else:
            losses.append(tf.abs(target_prices[i, num_steps - 1, 0] - base_prices[i, num_steps - 1, 0]))
    ...
    avg_loss = (tf.reduce_mean(profits) - tf.reduce_sum(profit_loss)) / num_of_losses
    ...
    if config.kelly: loss = loss - loss3
...
def create_estimator(config):
    ...
    model_dir = path_v4_0.model_dir + "/" + path_v4_0.market + "_" + str(config.predict_term) + "_" + \
    ...
    model_dir_cont = path_v4_0.model_dir + "/" + path_v4_0.market + "_" + str(config.predict_term) + "_" + str(config.step_interval) + "_" + str(config.num_steps) + "_" + \
...
def train(model, train_data, config):
    ...
    inputs, targets = reader_v4_0.producer(train_data, config.num_steps, config.step_interval, config.input_size, config.output_size)
    ...
    for i in range(len(targets)):
        for step in range(config.num_steps):
            if path_v4_0.Config().conversion == 'diff': target.append(targets[i, step, 1] - targets[i, step, 0])
            if path_v4_0.Config().conversion == 'rate': target.append(
                (targets[i, step, 1] - targets[i, step, 0]) / targets[i, step, 0] * 100)
            if path_v4_0.Config().conversion == 'norm': target.append((targets[i, step, 0] - 67) / 30)
...
def test(model, test_data, config):
    ...
    inputs, targets = reader_v4_0.producer(test_data, config.num_steps, config.step_interval, config.input_size, config.output_size)
...
def predict(model, predict_data, config, test_start_index):
    ...
    inputs, targets = reader_v4_0.producer(predict_data, config.num_steps, config.step_interval, config.input_size, config.output_size)
    ...
    index_close, index_open, date, today, std = reader_v4_0.make_index_date(predict_data, config)
