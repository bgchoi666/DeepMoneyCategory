...
def no_gradual_train(k, M2M, config):
    ...
    # log file setup
    log = set_log(path_v3_3.log_dir)

    log.info(datetime.now().strftime("%Y %m %d %H %M %S"))
    log.info("====================== no gradual train start ===========================")

    #create datasets from the given file
    rf, config = reader_v3_3.norm_data(config)
    dataset = reader_v3_3.get_dataset(rf, config)
    ...
    pd.DataFrame.from_dict(tmp_results).to_csv("ensemble" + str(k) + path_v3_3.tmp_file_name, index=False)
...
def gradual_train(k, M2M, config):
    ...
    while config.train_end1_index >= config.test_end_index:
        ...
        if first_gradual:
            ...
            #create datasets from the given file
            rf, config = reader_v3_3.norm_data(config)
            train_data1, train_data2, test_data, predict_data, test_start_index = reader_v3_3.get_dataset(rf, config)
        else:
        ...
        if not first_gradual and os.path.isfile("ensemble" + str(k) + path_v3_3.tmp_file_name):
            # read the saved temporary file
            tmp = pd.read_csv("ensemble" + str(k) + path_v3_3.tmp_file_name, encoding="ISO-8859-1")
...
def main(_):
    ...
    ensemble = path_v3_3.Config().ensemble
    gradual = path_v3_3.Config().gradual

    log.info("train term 1:" + (path_v3_3.Config()).train_start1 + "~" + (path_v3_3.Config()).train_end1)
    log.info("train term 2:" + (path_v3_3.Config()).train_start2 + "~" + (path_v3_3.Config()).train_end2)
    ...
    if ensemble:
       ...
       tot_num_thread = path_v3_3.num_threads
       ...
       config = [path_v3_3.Config() for i in range(tot_num_thread)] #get configuration class as many as the number of threads
       ...
       res = [pd.read_csv("ensemble" + str(i) + path_v3_3.tmp_file_name, encoding="ISO-8859-1") for i in range(path_v3_3.num_threads)]
       ...
       for j in range(len(res[0]["date_base"])):
           for i in range(path_v3_3.num_threads):
               index_pred_avg[j] += res[i]["index_pred_" + str(i)].values[j] / path_v3_3.num_threads
       ...
       if not os.path.isdir(path_v3_3.result_dir) :
           os.makedirs(path_v3_3.result_dir)
       result_file = path_v3_3.result_dir + "/" + path_v3_3.market + "_" + "alpha" + str(config[0].alpha) + "beta" + str(config[0].beta) + \
                     "_ensemble_" + str(config[0].predict_term) + \
                     str(config[0].rnn_mode) + "_" + str(config[0].hidden_size1) + "_" + \
       ...
       res = pd.read_csv("ensemble0" + path_v3_3.tmp_file_name, encoding="ISO-8859-1")
    
       # if not existing, create a new result direcctory
       if not os.path.isdir(path_v3_3.result_dir) :
           os.makedirs(path_v3_3.result_dir)
       result_file = path_v3_3.result_dir + "/" + path_v3_3.market + "_" + str(config.predict_term) + "_" + \
       ...
       os.rename("ensemble0" + path_v3_3.tmp_file_name, result_file)
