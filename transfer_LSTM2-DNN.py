...
checkpoint_path = model_name + "/input44_" + "future" + str(future_day) + "_batch20_unit100_DNN.ckpt"
checkpoint_path_best = model_name + "/input44_" + "future" + str(future_day) + "_batch20_unit100_DNN_best.ckpt"
...
def save(test_prediction, test_y, test_dates, test_base_prices):
    ...
    result.train_predict_price = result.test_predict_price
    # 실재값 복원
    result.test_output_price = back_to_price(result.test_output, test_base_prices, target_type)
    result.train_output_price = result.test_output_price
