...
import reader_v5_0 as reader
import path_v5_0 as path
...
def model_fn(features, labels, mode, params):
    ...
    if config.RRL and config.alpha != 0:
        ...
        for i in range(batch_size):
            for j in range(num_steps):
                R.append((abs(target_prices[i, j, 0] - base_prices[i, j, 0]) * F[i, j, 0] - 0.00003 * abs(target_prices[i, j, 0] - base_prices[i, j, 0]))/base_prices[i, j, 0])
        R = tf.reshape(R, [batch_size, num_steps, 1])
        ...
        # calc. downside sharp ratio
        loss2 = tf.reduce_mean(R) / tf.keras.backend.std(DR)
    else:
        loss2 = 0
    ...
    if config.kelly and config.beta != 0:
...
def create_estimator(config, dir):
    ...
    if dir == "" or config.model_reset == True:
        try:
