...
def create_estimator(config, dir):
    ...
    if dir == "":
        ...
        model_dir = path.model_dir + "/" + path.market + "_" + str(config.predict_term) + "day_" + \
                    "a" + str(config.alpha) + "b" + str(config.beta) + "_" + \
...
def predict(model, predict_data, config, test_start_index):
    ...
    if config.base == "open": base_price = index_open
    else: base_price = index_close
    for i in range(len(index_close)):
        if target[i] != 'Nan':
            ...
            # set the profits to zero if the base date is holiday
            if i > 0 and base_price[i] == base_price[i - 1]:
               profits.append(0)
            else:
                if config.conversion == 'sign' :
                    if pred_values[i] > 0:
                        profits.append(index_real[i] - base_price[i])
                    else:
                        profits.append(base_price[i] - index_real[i])
                else:
                    if index_pred[i] - base_price[i] > 0:
                        profits.append(index_real[i] - base_price[i])
                    else:
                        profits.append(base_price[i] - index_real[i])
        else:
            ...
            if config.conversion == 'sign':
                index_pred.append(index_close[i] * (pred_values[i]*0.1 + 1))
    return today, date, index_pred, index_real, base_price, profits, pred_values, target_values, std, asset_rebal
