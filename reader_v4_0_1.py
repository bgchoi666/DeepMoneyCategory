...
import path_v4_0_1 as path
from statistics import stdev

def norm_data(config):
  """
  read normalized data, return config fixed with train, test start-end index in raw dataframe

  kospi200 futures n days-forward  predictions text files with 943 input features,
  ...
  Returns:
    dataframe, adjusted config with start-end indices
  """
...
def get_dataset(raw_df, config):
  """
  make train, test dataset from raw dataframe

  kospi200 futures 5, 20, 65days-forward  predictions text files with 943 input features,
  and performs mini-batching of the inputs.

  Args:
    raw_df : raw dataframe with date, close, next-day open prices
    config : configuration class

  Returns:
    tuple (train_data1, train_data2, test_data, predict_data, config.test_start_index)
    where each of the data objects can be passed to producer function.
    * train_data[i] = input_data[i] + close_price[i] + close_price[i+predict_term]
  """
