    ...
    def validate(self, loss):
        if self._loss < loss:
            ...
            if self._step > self.patience:
                if self.verbose:
                    print('Training process is stopped early....step ', self__step)
                return True
        else:
            self._step = 0
            self._loss = loss
...
def next_random_interval_batch(X_data, y_data, batch_size, future_day):
    ...
    sel_idx = []
    for i in range(idx, idx + (batch_size-1)*future_day, future_day):
        sel_idx.append(i)
...
class GenerateResult():
    def __init__(self, test_predict1, test_predict2, test_output, test_dates, n_timestep, future_day, train_end_back, trans_day):
        ...
        self.train_end_back = train_end_back
    ...
    def table(self, open, high, low, profits1, profits2):
        self.result_table = pd.DataFrame({"date":self.test_dates, "real":self.test_output.reshape(-1), 
                                           ...
                                           "손익":profits1, "손절":profits2}).reset_index(drop=True)
        
        return self.result_table

    
    def evaluation(self, test_pred, test_output): #평가 지표 생성
        self.test_eval = Evaluate(test_pred, test_output)
    ...
    def save_result(self,model_name,item_name,n_units,target_type,batch_size,n_timestep,time_interval,epochs,alpha,comment):
        ...
        self.info = model_name+'_'+item_name+'_'+datetime.now().strftime("%m-%d:%H:%M")+'_'+str(self.future_day)+\
        ...
        sheet1.cell(8,13,'units 수')
        sheet1.cell(9,13,'Prediction day')
        sheet1.cell(10,13,'실재 예측일')
        sheet1.cell(11,13,'Transfer day')
        sheet1.cell(12,13,'epochs')
        sheet1.cell(13,13,'alpha')
        sheet1.cell(14,13,'commnent')
        sheet1.cell(15,13,'datetime')
        ...
        sheet1.cell(8,13, n_units)
        sheet1.cell(9,14, self.future_day)
        sheet1.cell(10,14, -self.train_end_back)
        sheet1.cell(11,14, self.trans_day)
        sheet1.cell(12,14, epochs)
        sheet1.cell(13,14, alpha)
        sheet1.cell(14,14, comment)
        sheet1.cell(15,14, datetime.now().strftime('%Y-%m-%d-%H:%M:%S'))
        
        
        sheet1.cell(1,15, '실험결과')
        sheet1.cell(2,15, 'Training Set')
        sheet1.cell(2,16, 'TRUE')
        sheet1.cell(3,15, 'PREDICTION')
        sheet1.cell(3,16, '0')
        sheet1.cell(3,17, '1')
        sheet1.cell(4,15, '0')
        sheet1.cell(5,15, '1')
        ...
        sheet1.cell(7,15, 'Test Set')
        sheet1.cell(7,16, 'TRUE')
        sheet1.cell(8,15, 'PREDICTION')
        sheet1.cell(8,16, '0')
        sheet1.cell(8,17, '1')
        sheet1.cell(9,15, '0')
        sheet1.cell(10,15, '1')
        ...
        sheet1.cell(9,16,self.test_confus_mat[0][0])
        sheet1.cell(9,17,self.test_confus_mat[0][1])
        sheet1.cell(10,16,self.test_confus_mat[1][0])
        sheet1.cell(10,17,self.test_confus_mat[1][1])
        

     
        
        sheet1.cell(9,18,'Rise')
        sheet1.cell(10,18,'Fall')
        sheet1.cell(8,19,'Precision')
        sheet1.cell(8,20,'Recall')
        
        sheet1.cell(9,19,self.test_precision_rise)
        sheet1.cell(9,20,self.test_recall_rise)
        
        sheet1.cell(10,19,self.test_precision_fall)
        sheet1.cell(10,20,self.test_recall_fall)
        
        
        sheet1.cell(3,23,'Training')
        sheet1.cell(3,24,'Test')
        
        sheet1.cell(4,22,'MAPE')
        sheet1.cell(5,22,'MSE')
        sheet1.cell(6,22,'Accuracy')
        
       
        sheet1.cell(4,24,self.test_MAPE)
        
        sheet1.cell(5,24,self.test_MSE)
        
        sheet1.cell(6,24,self.test_accu)
